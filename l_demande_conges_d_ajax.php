...
$reponse = $db->query("SELECT demande_conges.`id`,  CONCAT(DATE_FORMAT(
    demande_conges.`date_debut`, '%d'), '/', DATE_FORMAT(demande_conges.`date_debut`, '%m'),'/', DATE_FORMAT(demande_conges.`date_debut`, '%Y')),  CONCAT(DATE_FORMAT(demande_conges.`date_fin`, '%d'), '/', DATE_FORMAT(demande_conges.`date_fin`, '%m'),'/', DATE_FORMAT(demande_conges.`date_fin`, '%Y')), `nbr_jour`, demande_conges.`motif`, demande_conges.`statut`, type_conges.type_conges, CONCAT(DATE_FORMAT(demande_conges.`date_enregistrement`, '%d'), '/', DATE_FORMAT(demande_conges.`date_enregistrement`, '%m'),'/', DATE_FORMAT(demande_conges.`date_enregistrement`, '%Y')), demande_conges.etat, employe.prenom, employe.nom, departement.nom
    FROM `demande_conges` 
...
while ($donnees = $reponse->fetch()) {
    ...
    echo '
        ...
        <a  onclick="return(confirm(\'Voulez-vous vraiment accepter cette demande ?\'))" class="btn btn-success btn-sm" data-mdb-ripple-init data-toggle="modal" data-target="#modalAccepter' . $id . '">Accepter</a>
        <br>
        <br>
    ...
    echo '<td>';
?>
    <div class="modal fade" id="modalAccepter<?= $id ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog cascading-modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body mb-0">
                    <form method="POST" action="validation_demande_conges_d.php">
                        <input type="number" value="<?= $id ?>" name="id_demande" hidden>
                        <div class="row">
                            <div class="col-md-12">
                                A déduire du solde des congès
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="a_deduire" id="inlineRadio2" value="1" checked/>
                                    <label class="form-check-label" for="inlineRadio2">Oui</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="a_deduire" id="inlineRadio1" value="0"  />
