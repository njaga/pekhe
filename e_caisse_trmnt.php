...
$reponse = $db->prepare("INSERT INTO `caisse`(`date_operation`, `type`, `section`, `bon`, carnet, `motif`, `montant`, id_user) VALUES (?,?,?,?,?,?,?,?)");
$reponse->execute(array($date_operation, $type_operation, $section, $bon, $carnet, $description, $montant, $_SESSION['id_vigilus_user'])) or die(print_r($reponse->errorInfo()));
...
switch ($_FILES["bc"]['error']) {
    ...
    default: {
            ...
            if (!in_array($extension, $extensions_autorisees)) {
                ...
                $req_document = $db->prepare("INSERT INTO document_caisse (`type_document`, `nom_document`, `chemin`, `id_caisse`,  id_user) VALUES (?,?,?,?,?)");
                $req_document->execute((array("PJ", "PJ", $doc_caisse, $id_caisse,  $_SESSION['id_vigilus_user'])));
            }
        }
}
