...
WHERE employe.etat=1 and employe.id NOT IN (SELECT planning_agent.id_agent from planning_agent WHERE planning_agent.etat=1) ");
$records = $stmt->fetch();
...
FROM `employe` 
INNER JOIN departement ON employe.id_departement = departement.id
WHERE employe.etat=1 and employe.id NOT IN (SELECT planning_agent.id_agent from planning_agent WHERE planning_agent.etat=1) " . $searchQuery);
...
FROM `employe` 
INNER JOIN departement ON employe.id_departement = departement.id
WHERE employe.etat=1 and employe.id NOT IN (SELECT planning_agent.id_agent from planning_agent WHERE planning_agent.etat=1) " . $searchQuery . " ORDER BY " . $columnName . " " . $columnSortOrder . " LIMIT :limit,:offset");
