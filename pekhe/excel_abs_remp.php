...
$phpExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('C1', 'Nbr Jour Supplementaire')
    ->setCellValue('D1', 'Heures Supp')
    ->setCellValue('E1', 'Nbr Jour Absence')
    ->setCellValue('F1', 'Absences')
    ->setCellValue('G1', 'Accomptes')
    ->setCellValue('H1', 'Motif(s) absence(s)')
    ->setCellValue('I1', 'Motif(s) heures supp');
...
while ($donnees_emp = $rep_emp->fetch()) {
    ...
    //motifs heures supplémentaires
    $req_motif_hs = $db->prepare("SELECT CONCAT(DATE_FORMAT(`date_absence`, '%d'), '/', DATE_FORMAT(`date_absence`, '%m'),'/', DATE_FORMAT(`date_absence`, '%Y')), motif_hs
        FROM `absence_remplacement` 
        WHERE id_employe_remplacant=? AND date_absence BETWEEN ? AND ?");
    $req_motif_hs->execute(array($id_employe, $date_debut, $date_fin));
    while ($donnees_motif_hs = $req_motif_hs->fetch()) {
        $motif_hs = $motif_hs . "" . $donnees_motif_hs['0'] . " - " . $donnees_motif_hs['1'] . "\n";
    }

    //motifs absences
    $req_motif_absence = $db->prepare("SELECT CONCAT(DATE_FORMAT(`date_absence`, '%d'), '/', DATE_FORMAT(`date_absence`, '%m'),'/', DATE_FORMAT(`date_absence`, '%Y')), motif_absence
        FROM `absence_remplacement` 
        WHERE id_employe_remplace=? AND date_absence BETWEEN ? AND ?");
    $req_motif_absence->execute(array($id_employe, $date_debut, $date_fin));
    while ($donnees_motif_abs = $req_motif_absence->fetch()) {
        $motif_abs = $motif_abs . "" . $donnees_motif_abs['0'] . " - " . $donnees_motif_abs['1'] . "\n";
    }

    //nbr absence
    $req_nbr_abs = $db->prepare("SELECT count(*) 
        FROM `absence_remplacement`
        WHERE id_employe_remplace=? AND date_absence BETWEEN ? AND ?");
    $req_nbr_abs->execute(array($id_employe, $date_debut, $date_fin));
    $donnees_nbr_abs = $req_nbr_abs->fetch();
    $nbr_abs = $donnees_nbr_abs['0'];

    //nbr hs
    $req_nbr_hs = $db->prepare("SELECT count(*) 
        FROM `absence_remplacement`
        WHERE id_employe_remplacant=? AND date_absence BETWEEN ? AND ?");
    $req_nbr_hs->execute(array($id_employe, $date_debut, $date_fin));
    $donnees_nbr_hs = $req_nbr_hs->fetch();
    $nbr_hs = $donnees_nbr_hs['0'];


    $phpExcel->setActiveSheetIndex(0)
        ->setCellValue('A' . $i, $employe)
        ->setCellValue('B' . $i, $site)
        ->setCellValue('C' . $i, $nbr_hs)
        ->setCellValue('D' . $i, $montant_hs)
        ->setCellValue('E' . $i, $montant_retirer)
        ->setCellValue('F' . $i, $nbr_abs)
        ->setCellValue('G' . $i, $montant_accompte)
        ->setCellValue('H' . $i, $motif_abs)
        ->setCellValue('I' . $i, $motif_hs);
