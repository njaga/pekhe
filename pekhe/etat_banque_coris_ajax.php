...
if ($mois == 1) {
    ...
    $reponse = $db->prepare("SELECT (SELECT COALESCE(SUM(montant),0) FROM banque WHERE type='entree' AND date_operation<?)  - (SELECT COALESCE(SUM(montant),0) FROM banque WHERE compte='BIS' AND type='sortie' AND  date_operation<?)");
    ...
    //Solde su mois précédent
    $date_dernier_jour = $annee . '-' . $mois . '-01';
    $reponse = $db->prepare("SELECT (SELECT COALESCE(SUM(montant),0) FROM banque WHERE compte='BIS' AND type='entree' AND date_operation<?)  - (SELECT COALESCE(SUM(montant),0) FROM banque WHERE type='sortie' AND  date_operation<?)");
    $reponse->execute(array($date_dernier_jour, $date_dernier_jour));
    $donnees = $reponse->fetch();
    $solde = $donnees['0'];
    $solde_init = $donnees['0'];
...
$reponse = $db->prepare("SELECT banque.id, CONCAT(DATE_FORMAT(`date_operation`, '%d'), '/', DATE_FORMAT(`date_operation`, '%m'),'/', DATE_FORMAT(`date_operation`, '%Y')), `type`, `section`, `bon`, `motif`, `montant`, date_operation, carnet, document_caisse.nom_document, document_caisse.chemin, num_cheque
FROM `banque`
LEFT JOIN document_caisse ON banque.id=document_caisse.id_banque
WHERE banque.compte='CORIS' AND banque.etat=1 AND MONTH(date_operation)=? AND YEAR(date_operation)=? AND motif like CONCAT('%', ?, '%') ORDER BY banque.date_operation ASC");
...
if ($nbr > 0) {
    while ($donnees = $reponse->fetch()) {
        ...
        echo '<td>
            <a href="" data-toggle="modal" data-target="#modalModifSite' . $id . '" class="teal-text" data-toggle="tooltip" data-placement="top" title="Modifier"><i class="fas fa-pencil-alt"></i></a> <a href="s_ligne_banque.php?id=' . $id . '" class="red-text" onclick="return(confirm(\'Voulez-vous supprimer cet dépense ?\'))"><i class="fas fa-times"></i></a>
                ...
                <div class="modal-dialog cascading-modal" role="document">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body mb-0">
                            <form method="POST" action="m_banque_trmnt.php" enctype="multipart/form-data">
                                <input type="number" class="id" name="id" value="<?= $id ?>" hidden>
                                <div class="row">
                                    <div class="col-md-5 col-sm-8">
                                        <div class="md-form">
                                            <input type="text" id="date_operation" value="<?= $date_operation1 ?>" name="date_operation" class="form-control datepicker" required>
                                ...
                                <div class="row">
                                    <div class="col-md-5 col-sm-8">
                                        <div class="md-form">
                                            <input type="number" value="<?= $num_cheque ?>" id="num_cheque" name="num_cheque" class="form-control">
