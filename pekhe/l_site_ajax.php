...
$search = $_POST['search'];
if ($search == "") {
	$reponse = $db->query("SELECT `id`, `nom`, `localisation`, `id_departement`, CONCAT(DATE_FORMAT(date_debut, '%d'), '/', DATE_FORMAT(date_debut, '%m'),'/', DATE_FORMAT(date_debut, '%Y')), date_debut
    FROM `site` 
    WHERE etat=1 AND id_departement=3
    ORDER BY nom DESC");
} else {
	$reponse = $db->prepare("SELECT `id`, `nom`, `localisation`, `id_departement`, CONCAT(DATE_FORMAT(date_debut, '%d'), '/', DATE_FORMAT(date_debut, '%m'),'/', DATE_FORMAT(date_debut, '%Y')), date_debut
    FROM `site` 
    WHERE CONCAT (nom,' ',localisation) like CONCAT('%', ?, '%') AND etat=1 AND id_departement=3
    ORDER BY nom DESC");
...
while ($donnees = $reponse->fetch()) {
	...
	if ($_SESSION['profil_vigilus_user'] == "ass_op") {
		echo '<td class="text-center"><a href="planning_detail.php?s=' . $id . '" class="blue btn" data-toggle="tooltip" data-placement="top" title="Détails">Plannig</a></td>';
		echo '
			<td>
				...
				&nbsp&nbsp&nbsp<a href="" data-toggle="modal" data-target="#modalSuppSite' . $id . '" class="red-text" data-toggle="tooltip" data-placement="top" title="Supprimer"><i class="fas fa-times"></i></a>
			</td>';
	}
	$i++;
