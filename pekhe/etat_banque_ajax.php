...
if ($mois == 1) {
    ...
    $reponse = $db->prepare("SELECT (SELECT COALESCE(SUM(montant),0) FROM banque WHERE type='entree' AND date_operation<?)  - (SELECT COALESCE(SUM(montant),0) FROM banque WHERE type='sortie' AND  date_operation<?)");
...
$reponse = $db->prepare("SELECT banque.id, CONCAT(DATE_FORMAT(`date_operation`, '%d'), '/', DATE_FORMAT(`date_operation`, '%m'),'/', DATE_FORMAT(`date_operation`, '%Y')), `type`, `section`, `bon`, `motif`, `montant`, date_operation, carnet, document_caisse.nom_document, document_caisse.chemin
FROM `banque`
LEFT JOIN document_caisse ON banque.id=document_caisse.id_banque
WHERE banque.etat=1 AND MONTH(date_operation)=? AND YEAR(date_operation)=? AND motif like CONCAT('%', ?, '%') ORDER BY banque.date_operation ASC");
...
if ($nbr > 0) {
    while ($donnees = $reponse->fetch()) {
        ...
        echo '<td>
            ...
            <a href="s_ligne_banque.php?id=' . $id . '" class="red-text" onclick="return(confirm(\'Voulez-vous supprimer cet dépense ?\'))"><i class="fas fa-times"></i></a>
            &nbsp&nbsp&nbsp
