...
$mois = $_POST['mois'];
$annee = $_POST['annee'];
$annee = " AND YEAR(depense_projet.date_ajout)=" . $annee;
$mois = " AND MONTH(depense_projet.date_ajout)=" . $mois;
if ($search != "") {
    $search = " AND CONCAT(depense_projet.description, ' ', depense_projet.fournisseur) like CONCAT('%', " . $search . ", '%')";
} else {
    $search = "";
}
$reponse = $db->query("SELECT id, CONCAT(DATE_FORMAT(`date_ajout`, '%d'), '/', DATE_FORMAT(`date_ajout`, '%m'),'/', DATE_FORMAT(`date_ajout`, '%Y')) AS date_depense, description, qt, montant, fournisseur, depense_projet.etat, departement, priorite, etat 
FROM `depense_projet` 
WHERE depense_projet.etat>=0 " . $mois . " " . $annee);
...
while ($donnees = $reponse->fetch()) {
    ...
    if ($etat == "1" and $_SESSION['profil_vigilus_user'] == "comptabilite") {
        echo '
		<td>
        <a href="decaisser_depense.php?id=' . $id . '" class="btn btn-info btn-sm"><i class="fas fa-dollar-sign mr-2"></i>Décaissement</a>
        </td>';
    }
