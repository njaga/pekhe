...
$reponse = $db->prepare("UPDATE `banque` SET `date_operation`=?, `type`=?, `section`=?, `motif`=?, `montant`=?, num_cheque=? WHERE id=?");
$reponse->execute(array($date_operation, $type_operation, $section, $description, $montant, $num_cheque, $id)) or die(print_r($reponse->errorInfo()));
$nbr = 1;
...
switch ($_FILES["bc"]['error']) {
    ...
    default: {
            ...
            if (!in_array($extension, $extensions_autorisees)) {
                ...
                $req_document = $db->prepare("UPDATE document_caisse SET `chemin` WHERE `id_banque`=?");
                $req_document->execute((array($doc_caisse, $id_banque)));
            }
        }
}
