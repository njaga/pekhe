...
$mois = $_POST['mois'];
$annee = $_POST['annee'];
$annee = " AND YEAR(retour_art_gard.date_retour)=" . $annee;
if ($mois != "tous") {
    $mois = " AND MONTH(retour_art_gard.date_retour)=" . $mois;
}
if ($search != "") {
...
if ($mois == "tous") {
    $reponse = $db->query("SELECT retour_art_gard.id, CONCAT(DATE_FORMAT(`date_retour`, '%d'), '/', DATE_FORMAT(`date_retour`, '%m'),'/', DATE_FORMAT(`date_retour`, '%Y')), CONCAT(employe.prenom,' ', employe.nom), `lacoste`, `veste_normale`, `veste_parka`, `chaussure_ville`, `chaussure_securite`, `tonfa`, `ceinturon`, `epaulettes`, `chemise`, `pantalon_simple`, `badge`, `kepi`, `casquette`, `cravate`, `combinaison`, blouson, pantalon_parka, retour_art_gard.commentaire 
    FROM `retour_art_gard` 
    INNER JOIN employe ON employe.id = retour_art_gard.agent
    WHERE retour_art_gard.etat=1 " . $annee . " " . $search . " ORDER BY date_retour DESC");
} else {
    $reponse = $db->query("SELECT retour_art_gard.id, CONCAT(DATE_FORMAT(`date_retour`, '%d'), '/', DATE_FORMAT(`date_retour`, '%m'),'/', DATE_FORMAT(`date_retour`, '%Y')), CONCAT(employe.prenom,' ', employe.nom), `lacoste`, `veste_normale`, `veste_parka`, `chaussure_ville`, `chaussure_securite`, `tonfa`, `ceinturon`, `epaulettes`, `chemise`, `pantalon_simple`, `badge`, `kepi`, `casquette`, `cravate`, `combinaison`, blouson , pantalon_parka, retour_art_gard.commentaire
FROM `retour_art_gard` 
INNER JOIN employe ON employe.id = retour_art_gard.agent
WHERE retour_art_gard.etat=1 " . $mois . " " . $annee . " " . $search . " ORDER BY date_retour DESC");
}
...
while ($donnees = $reponse->fetch()) {
    ...
    $commentaire = $donnees['20'];


    echo "<tr>";
    echo "<td class='text-center'><b>" . $i . "</b></td>";
    echo "<td class='text-center'>" . $date_retour . "</td>";
