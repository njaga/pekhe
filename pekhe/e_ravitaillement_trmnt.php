...
$reponse = $db->prepare("INSERT INTO `ravitaillement_art_gard`(`date_ravitailement`, `fournisseur`, `lacoste`, `veste_normale`, `veste_parka`, `chaussure_ville`, `chaussure_securite`, `tonfa`, `ceinturon`, `epaulettes`, `chemise`, `pantalon_simple`, `badge`, `kepi`, `casquette`, `registre`, `arme`, `chargeur_arme`, `casque`, `smartphone`, `telephone_simple`, `miroir_telescopique`, `detecteur`, `chargeur_detecteur`, gilet, cravate, combinaison, blouson, torche, pantalon_parka, `id_user`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
$reponse->execute(array($date_ravitailement, $fournisseur, $lacoste, $veste_normale, $veste_parka, $chaussure_ville, $chaussure_securite, $tonfa, $ceinturon, $epaulettes, $chemise, $pantalon_simple, $badge, $kepi, $casquette, $registre, $arme, $chargeur_arme, $casque, $smartphone, $telephone_simple, $miroir_telescopique, $detecteur, $chargeur_detecteur, $gilet, $cravate, $combinaison, $blouson, $torche, $pantalon_parka, $_SESSION['id_vigilus_user'])) or die(print_r($reponse->errorInfo()));
...
$reponse = $db->prepare("UPDATE `article_gardiennage` SET `qt`=qt + ? WHERE designation='Torche' ");
