...
$req=$db->prepare("UPDATE `accompte` SET `etat` = '2' WHERE `accompte`.`id` = ?");
...
if ($search == "") {
    ...
    INNER JOIN site ON affectation_site.id_site=site.id
    WHERE mois=? AND YEAR(date_demande)=?
    ...
    INNER JOIN affectation_site ON employe.id=affectation_site.id_employe
    INNER JOIN site ON affectation_site.id_site=site.id
    WHERE mois=? AND YEAR(date_demande)=? AND CONCAT(employe.prenom, ' ', employe.nom) like CONCAT('%', ?, '%')
    ORDER BY date_demande DESC");
...
while ($donnees = $reponse->fetch()) {
    ...
    if ($_SESSION['profil_vigilus_user']=="r_exp") {
        echo '
            <td>
