...
WHERE employe.etat=1 and employe.id NOT IN (SELECT dotation_art_gard.agent from dotation_art_gard WHERE dotation_art_gard.etat=1) ");
$records = $stmt->fetch();
...
FROM `employe` 
INNER JOIN departement ON employe.id_departement = departement.id
WHERE employe.etat=1 and employe.id NOT IN (SELECT dotation_art_gard.agent from dotation_art_gard WHERE dotation_art_gard.etat=1) " . $searchQuery);
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $db->prepare("SELECT employe.id, `matricule`, `prenom`, employe.nom AS nom_employe, CONCAT(DATE_FORMAT(`date_naissance`, '%d'), '/', DATE_FORMAT(`date_naissance`, '%m'),'/', DATE_FORMAT(`date_naissance`, '%Y')) AS date_naissance, `lieu_naissance`, `adresse`, `situation_matrimoniale`, `niveau_experience`, CONCAT(DATE_FORMAT(employe.date_debut, '%d'), '/', DATE_FORMAT(employe.date_debut, '%m'),'/', DATE_FORMAT(employe.date_debut, '%Y')) AS date_debut, `note`, `nbr_enfants`, departement.nom AS departement, `fonction`, employe.telephone
FROM `employe` 
INNER JOIN departement ON employe.id_departement = departement.id
WHERE employe.etat=1 and employe.id NOT IN (SELECT dotation_art_gard.agent from dotation_art_gard WHERE dotation_art_gard.etat=1) " . $searchQuery . " ORDER BY " . $columnName . " " . $columnSortOrder . " LIMIT :limit,:offset");
